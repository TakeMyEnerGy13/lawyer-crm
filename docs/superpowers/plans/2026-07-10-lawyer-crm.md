...
### Task 11: Карточка клиента (popup) + широкий контейнер

Контейнер 1040px, телефон nowrap. Клик по строке или карандашу → модальная карточка: имя, телефон, суть дела (textarea), статус, дата, Сохранить/Удалить, Esc/backdrop закрывают. Инлайн-правка строки удаляется (заменена карточкой). updateClient расширяется статусом.
